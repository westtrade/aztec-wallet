<script lang="ts">
	...
	import * as pkg from 'tonweb-mnemonic';

	const { generateMnemonic } = pkg;

	console.log(pkg);

	let words = writable<string[]>([]);

	onMount(async () => {
		words.set(await generateMnemonic());
	});
</script>
...
<div class="mx-auto flex min-h-fit max-w-sm grow flex-col items-center justify-center">
	...
	<h1 class="mb-8 text-center font-['Space_Grotesk'] text-3xl font-bold">24 secret words</h1>

	<p class="mb-2 text-center text-sm text-gray-500">
		Write down these 24 words in the correct <br /> order and store them in secret place.
	</p>

	<p class="mb-8 text-center text-sm text-gray-500">
		Use these words to restore access to <br />
		your wallet if you loose your password or <br />
	...
	<div class="grid w-full items-center justify-center gap-2">
		<Button size="lg" class="w-56 font-bold" href="/test-time">Continue</Button>
	</div>
</div>
